...
class Scorer(nn.Module):
    ...
    def score(self, path, latent) -> float:
        """
        Convenience API used by the inference runner for a single latent.
        """
        if latent is None:
            return 0.0
        if not isinstance(latent, torch.Tensor):
            latent = torch.tensor(latent, dtype=torch.float32)
        latent = latent.to(next(self.parameters()).device)
        with torch.no_grad():
            return float(self.forward(latent).reshape(-1)[0].item())
