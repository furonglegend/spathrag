...
class PathEncoder(nn.Module):
    ...
    def encode_path(self, path: List[Any], node_to_idx: Optional[Dict[Any, int]] = None, relations: Optional[List[Any]] = None) -> torch.Tensor:
        """
        Convenience API used by the inference runner for a single path.
        """
        rels = [relations] if relations is not None else None
        with torch.no_grad():
            return self.forward([path], node_to_idx=node_to_idx, rels=rels)[0]
