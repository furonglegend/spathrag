...
try:
    from models.gnn_encoder import RelationAwareGNN
except Exception:
    try:
        from src.models.gnn_encoder import RelationAwareGNN
    except Exception:
        RelationAwareGNN = None
...
try:
    from llm_integration.injection import project_path_latents_to_kv
except Exception:
    try:
...
try:
    from training.optim import get_optimizer, get_scheduler
except Exception:
    try:
...
def simple_data_loader(batch_size: int, num_batches: int = 10):
    ...
    if torch is None:
        for _ in range(num_batches):
            yield {"node_ids": [], "path_seq": [], "labels": []}
        return
    for _ in range(num_batches):
        # Synthetic tensors
        yield {
...
def pretrain_gnn_encoder(cfg: Dict, device: str):
    ...
    if torch is None:
        logger.info("PyTorch is not installed; skipping neural pretraining dry-run.")
        return
    path_encoder = PathEncoder(vocab_size=1000, embed_dim=32, rel_vocab_size=32) if PathEncoder is not None else None
    ...
    if path_encoder is not None:
        opt = get_optimizer(path_encoder, lr=cfg["pretrain"]["lr"]) if get_optimizer else torch.optim.Adam(path_encoder.parameters(), lr=cfg["pretrain"]["lr"])
    else:
        opt = None
    ...
    for e in range(epochs):
        ...
        for batch in dataloader:
            ...
            if path_encoder is not None:
                paths = [[int(x) for x in row[:4].tolist()] for row in batch["node_ids"]]
                emb = path_encoder(paths)
                loss = emb.pow(2).mean() * 0.0
                if opt:
                    opt.zero_grad()
                    loss.backward()
                    opt.step()
...
def train_scorer_and_injection(cfg: Dict, device: str):
    ...
    if torch is None:
        logger.info("PyTorch is not installed; skipping scorer/injection training dry-run.")
        return
    ...
    for e in range(epochs):
        ...
        for batch in dataloader:
            ...
            if scorer is not None:
                latents = torch.randn(
                    cfg["scorer_train"]["batch_size"],
                    4,
                    scorer.latent_dim,
                    device=device,
                )
                scores = scorer(latents)
...
def joint_finetune(cfg: Dict, device: str):
    ...
    if torch is None:
        logger.info("PyTorch is not installed; skipping joint fine-tuning dry-run.")
        return
...
def main(config_path: Optional[str] = None):
    ...
    if torch is None:
        logger.info("Running training dry-run without PyTorch. Install optional neural dependencies for real optimization.")
